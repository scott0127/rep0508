...
int main()
{
   ...
   i = ReadDataSize("Fig0424(a).bmp");
   data1 = (U_CHAR *)malloc( i );
   if (data1 == NULL) {
      exit(0);
   }
  
   ReadImageData("Fig0424(a).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
   ...
   double sin=0.70710678118;
   double cos=0.70710678118;
   //   
   for (i=0; i < biHeight; i++)
   {
       ...
       {        
         ...
         double jt=j-(biWidth/2);
         double it=i-(biHeight/2);
         double y = jt*sin+it*cos;
         //j2=>x
	      double x = jt*cos-it*sin;
         j2=x+(biWidth/2);
         i2=y+(biHeight/2);
   ...
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("new_3.bmp","wb") ) == NULL ){
