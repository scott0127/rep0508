...
int main()
{
   ...
   ReadImageData("new_3.bmp", bmpfileheader1, bmpinfoheader1, color_table1, data2);
   biWidth           =   GET_4B(bmpinfoheader1,4);
   biHeight          =   GET_4B(bmpinfoheader1,8);
   // Process the file
   biWidth4 = ((biWidth*1 +3)/4 *4);
   double sin=0.08715574274;
   double cos=0.99619469809;
   //   
   for (i=0; i < biHeight; i++)
   {
       ...
       for (j=0; j < biWidth; j++)
       {        
         data2[k+j] = data2[k+j]*data1[k+j]/255;
   ...
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("new_4.bmp","wb") ) == NULL ){
